/* =====================================================================
   ...
   ⚠️ 완료된 1회성 시드 — 비활성(#972). 입력이던 정적 `books.tsv`가 #490(canonical=Supabase)
   후속으로 제거돼 이 스크립트는 그대로는 재실행되지 않는다(아래 TSV 읽기에서 ENOENT).
   재시드가 필요하면 Supabase `books`에서 대상 도서를 export 해 입력으로 쓰도록 고친 뒤 실행할 것.

   (원래 동작) 민음사 전집(books.tsv ~492권)에 NPC 30명이 권당 2명씩 한 문장을 남긴다.
