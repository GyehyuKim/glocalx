...
5. **이슈 생성**: `POST /repos/GyehyuKim/readinggo/issues`
   ...
   - labels: `source:beta-inquiry` + 분류 라벨(`type:bug` / `type:feature` / `type:ux` / `type:question`, 기타·폴백은 `type:feedback`). 미존재 라벨은 생성 시 자동 생성됨
...
- ~~라벨 체계: 자동 분류(bug/feature) 라벨은 Phase 2 LLM 분류로.~~ **해결(#1105, §4.5)** — LLM 분류로 `type:bug`/`type:feature`/`type:ux`/`type:question` 부여(기타·폴백 `type:feedback`). 중복 문의 **병합**(같은 버그 묶기)은 여전히 후속.
