...
production 을 직전 양호 매니페스트로 되돌리는 법(서버 측):

```bash
# A) beta 가 아직 양호하면: 현재 beta 를 prod 로 다시 승격 (가장 단순)
#    → 단, beta 도 이미 나쁜 번들이면 쓰지 말 것.

# B) 특정 이전 버전으로 되돌리기 — 그 버전 매니페스트를 prod 키에 다시 쓴다.
...
- **개선 여지(열린 항목)**: 지금은 이전 버전 매니페스트를 KV 가 **자동 보관하지 않는다**(키 하나에 덮어쓰기). 안전한 원클릭 롤백을 위해 *직전 prod 매니페스트를 별도 키*(예: `ota:android:production:prev`)에 백업하는 한 줄을 `ota-promote.yml` 에 추가하는 것을 권장(후속). 현재는 Action 로그/R2 객체로 복원.
...
[ ] §4.1-B 로 prod 매니페스트를 직전 버전으로 되돌림
