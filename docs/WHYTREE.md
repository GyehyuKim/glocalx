...
graph BT
    ...
    %% M:N — GBP 자동화는 매출(유입)과 비용(절감) 둘 다에 기여
...
현재 부산에서 외국인 대상 음식점 마케팅을 전문으로 하는 서비스는 사실상 없다.
...
**대응 방안**  
구글이 단순 번역을 제공해도, GlocalX의 핵심은 번역이 아니라 **지속적 게시 자동화 + 한국 음식 카테고리 특화 콘텐츠**다.  
...
화살표가 두 갈래로 갈라지는 부분(매출 극대화 → 소상공인 지원 + 관광객 유치) 등 M:N 관계를 분석.

![종합 트리 - 정윤지](./whytree/whytree-yj.png)

### 이승원: HOW DOWN

"어떻게?"를 반복하며 위에서 아래로 구체화.  
우리 솔루션의 핵심 영역이 "온라인 환경 기본 셋팅"임을 명시.

![HOW DOWN - 이승원](./whytree/whytree-sw-howdown.png)

### 이승원: WHY UP

"왜?"를 반복하며 아래에서 위로 궁극 목적과 연결되는지 검증.  
5단 사다리 (Why → 중간 목적 → Who → How → What).
